...
function dataTbl = build_2Array_DataTable(arrayFile,singlediffFile,trial_Information,MVC,options)
    ...
    if isempty(fileInfo)
        ...
        dataTbl = append_No_DecompositionResults(dataTbl);
        dataTbl = repmat(dataTbl,size(trial_Information,1),1);
        dataTbl = [dataTbl, get_ForceTrace_Info(trial_Information,MVC)]; % RampStart, PlateauStart, MaxForce_N,TargetForce_N,TargetForce_MVC
        return;
    end
...
%         profile on
        ...
        ForceTrace_tbl   = get_ForceTrace_Info(trial_Information,MVC); % RampStart, PlateauStart, MaxForce_N,TargetForce_N,TargetForce_MVC
%         profile viewer
...
function dataTbl = append_No_DecompositionResults(dataTbl)

        %Decomp info
        dataTbl.ArrayNumber     = categorical({'None'});
        dataTbl.MU              = NaN;
        dataTbl.FiringTimes     = {[]};
