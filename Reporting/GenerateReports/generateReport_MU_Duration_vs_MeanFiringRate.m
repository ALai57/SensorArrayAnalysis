...
function generateReport_MU_Duration_vs_MeanFiringRate()
    ...
    print_All_MU_Duration_vs_MeanFiringRate(selection, MU_Data, options)
...
function analyses = get_Analyses()
    analyses.IndividualSubject{1} = @(selection,subjData,options)print_Subject_MU_Duration_vs_MeanFiringRate(selection,subjData,options);
end
...
function print_ReportDescription(selection)
    ...
    selection.Font.Size = 16;
    selection.Font.Bold = 0;
    selection.TypeText(['This report contains a summary of MU Duration vs. MU Mean Firing Rate.' char(13)])  
