...
%     MU_Data = append_STA_Window_Statistics_ToSTATable([],options);  %ONLY FOR THRESHOLDING
    ...
    report.IndividualSubject{1} = @(selection,subjData,options)print_Subject_MU_MeanFiringRate_vs_ThresholdForce(selection,subjData,options);
    
    [serverHandle, selection] = open_ConnectionToWord();
    
    report_Description(selection);
    print_OptionsAndAnalyses(selection, options, report);
    report_BySubject_Table(selection, MU_Data, report.IndividualSubject,options);
...
%     options.Trial.Analysis{2} = @(trial_Data,options)calculate_MU_MeanFiringRate_FromTrial(trial_Data,options);
