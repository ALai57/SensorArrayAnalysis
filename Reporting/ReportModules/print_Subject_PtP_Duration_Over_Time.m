...
function print_Subject_PtP_Duration_Over_Time(selection,subjData)
    ...
    hM = custom_heatmap(n0(:,2:end-1),edges(:,2:end-1),1:9);
    xlabel('MU duration (ms)');
    ylabel('Time Epoch')
    SID = char(subjData.SID(1));
    title({['Subject = ' SID '. All Motor units'], 'Distribution of Peak to Peak Duration over time (from STA)'})
    ...
    title('Distribution of Peak to Peak duration over time (CV)')
